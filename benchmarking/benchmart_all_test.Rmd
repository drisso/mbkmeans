...
cluster_wcss_calcu<-function(dat, centro,label){
    nd = nrow(dat)
    nc= nrow(centro)
...
cluster_wcss<-function(dat,centro,label){
    nr=nrow(centro)
    wcss_final<-matrix(0,100,nr)
    for(i in 1:100){
        
        wcss_final[i,]=cluster_wcss_calcu(dat,centro,label)
    }
    
    fianl = colMeans(wcss_final)
}
...
ARI_cal<-function(data,cluster){
    ari<-numeric(100)
    for(i in 1:100){
       ari[i]<-adjustedRandIndex(data,cluster)
    }
