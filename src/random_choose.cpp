...
//SEXP calc_top_features (SEXP matrix, int init_fraction) {
  //BEGIN_RCPP
  //auto mattype1=beachmat::find_sexp_type(matrix);
  //if (mattype1==INTSXP) {

    //auto mat1=beachmat::create_integer_matrix(matrix);
    //return random_choose<int, Rcpp::IntegerMatrix>(mat1.get(),init_fraction);
  //} else if (mattype1==REALSXP) {

    //auto mat1=beachmat::create_numeric_matrix(matrix);

    //return random_choose<double, Rcpp::NumericMatrix>(mat1.get(), init_fraction);
  //} else {
    //throw std::runtime_error("unacceptable matrix type");
  //}
  //END_RCPP
